...
var baseSchemaGrammar = req('knex/clients/base/schemagrammar').baseSchemaGrammar;

// Grammar for the schema builder.
exports.schemaGrammar = _.defaults({

  // The possible column modifiers.
  modifiers: ['Nullable', 'Default', 'Increment', 'After', 'Comment'],
  ...
  handleResponse: function(builder, response) {
    if (builder.type === 'tableExists') return response[0].length > 0;
    if (builder.type === 'columnExists') return response[0].length > 0;
    return response;
  },
  ...
  compileTableExists: function(builder) {
    return 'select * from information_schema.tables where table_catalog = ? and table_name = ?';
  },
  ...
  modifyIncrement: function(builder, column) {
    if (column.autoIncrement && (column.type == 'integer' || column.type == 'bigInteger')) {
      return ' not null identity(1,1) primary key';
    }
  },
